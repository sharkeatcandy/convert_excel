...
(async () => {
  ...
  const common = [["C2", "B"], ["H10", "C"], ["H3", "D"], ["C5", "E"], ["C4", "F"]];
  ...
  target.getCell(`T${targetRow}`).value = get("H5");
  targetRow += 1;
  ...
  assert.deepStrictEqual(warnings, ["F24:115.05.06 Z19557293"]);
  ...
  assert.strictEqual(result.getCell("U2").value, null);
  assert.strictEqual(result.getCell("G3").value, null);
  assert.strictEqual(result.getCell("H3").value, "金驊");
  ...
  assert.strictEqual(result.getCell("P3").value, null);
  assert.strictEqual(result.getCell("T3").value, null);
  assert.strictEqual(result.getCell("U3").value, 3150);
  assert.deepStrictEqual(result.getCell("B1").style, headerStyle);

  console.log("PASS: 1 crown row + 1 legacy vendor row; identifiers, mappings, and header styles survived XLSX round-trip");
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
